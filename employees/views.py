...
class AttendanceAPIView(viewsets.ModelViewSet):
    ...
    filter_backends = [filter.DjangoFilterBackend , filters.SearchFilter , filters.OrderingFilter]
    filterset_fields = ['employee', 'date', 'status', 'check_in_status', 'check_out_status']
    search_fields=['employee__first_name', 'employee__middle_name', 'employee__last_name' , 'date', 'status', 'check_in_status', 'check_out_status']
    ordering_fields = ['employee', 'date', 'status', 'check_in_status', 'check_out_status']
    ...
    def check_out(self,request):
        ...
        if not active_log:
            return Response({'error': 'No active check in log found. Maybe already checked out?' ,'employee':employee.first_name + " " + employee.middle_name + " " + employee.last_name},status=status.HTTP_400_BAD_REQUEST) 
        now=timezone.now()
        today_date=now.date()
        today=now.weekday()
        schedule=WorkSchedule.objects.filter(is_active=True).first()
        if not schedule:
            return Response({'error':'no active schedule found' , 'employee':employee.first_name + " " + employee.middle_name + " " + employee.last_name},status=status.HTTP_404_NOT_FOUND)
        try:
            work_day=WorkDay.objects.get(schedule=schedule,day=today)
        except WorkDay.DoesNotExist:
            return Response({'error':'No work schedule for today' , 'employee':employee.first_name + " " + employee.middle_name + " " + employee.last_name},status=status.HTTP_404_NOT_FOUND)
...
class SalaryAPIView(generics.ListAPIView):
    ...
    filter_backends = [filter.DjangoFilterBackend , filters.SearchFilter , filters.OrderingFilter]
    filterset_fields = ['employee__first_name' , 'employee' ,'month','year','base_salary','final_salary','absent_days','unpaid_vecation_days','late_or_left_early_count','generated_at']
    search_fields=['employee__first_name'  ,'month','year','base_salary','final_salary','absent_days','unpaid_vecation_days','late_or_left_early_count','generated_at']
    ordering_fields = ['employee__first_name' , 'employee' ,'month','year','base_salary','final_salary','absent_days','unpaid_vecation_days','late_or_left_early_count','generated_at']
